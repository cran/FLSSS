...
unsigned giveBirth(PAT&child,NumericVector&v,double&ME,std::vector<std::vector<double> >&M){
...
void TTT_stack_1_1(unsigned&LEN,NumericVector&v,double&ME,
                ...
                std::vector<std::vector<double> >&M, std::vector<PAT>&SK, double&Duration){
if(LEN==1)
{
...
if(SK.front().UBI.size()==0&&SK.size()<2)return;
unsigned boo;
...
{
    xi=common.begin();
    unsigned i=child.LBI.back()-v.begin()+1, end=child.UBI.back()-v.begin()+1, tmp;
    if(i>end){tmp=i;i=end;end=tmp;}
    for(std::vector<PAT>::iterator SKi=SK.begin()+1;SKi!=SK.end();++SKi,++xi)*xi=SKi->s-v.begin()+1;
    for(;i<=end;i++)
    {
        common.back()=i;
...
else if(boo==2)
{
    xi=common.begin();
    for(std::vector<PAT>::iterator SKi=SK.begin()+1;SKi!=SK.end();++SKi,++xi)*xi=SKi->s-v.begin()+1;
    std::vector<NumericVector::iterator>::iterator i=child.UBI.begin();
    for(;i!=child.UBI.end();++i,++xi)*xi=*i-v.begin()+1;
    result.push_back(common);
}
while(update(SK.back())==0)
{
    SK.pop_back();
    if(SK.size()<2)return;
}
...
theMatrix(v,len,M);
TTT_stack_1_1(len,v,ME,result,sizeNeeded,M,SK,Duration);
...
std::vector<unsigned>I(3*(SK.size()-1));
...
NumericVector::iterator vbegin=v.begin();
unsigned SKsize=I.size()/3+1;
std::vector<PAT>SK(SKsize);
std::vector<PAT>::iterator pati=SK.begin()+1;
...
for(unsigned i=0;i<result.size();i++)lis[i]=result[i];
pati=SK.begin()+1;
std::vector<unsigned>II(3*(SK.size()-1));
